...
2. **Benchmarks.** If bench drift was observed, update `bench/baseline.json` with a note explaining the change.
3. **SR matrix.** Fill the §5 results row in `docs/m6-hardening/sr-matrix.md` for the new version. Any FAIL cell requires a linked issue.
4. **Version bump.** Update the version in all four `packages/*/package.json` files.
   ```bash
...
6. **Publish:**
   ...
   ```
7. **Update the changelog.** Add the v1.0.0 entry at the top of `CHANGELOG.md`.
